...
This will:
    ...
    3. Save to designs/{sanitized_name}/Simulation.data.csv
"""
...
SAMPLE_CONFIGS_DIR = os.path.join(_backend_dir, "sample_configs")
DESIGNS_DIR = os.path.join(_backend_dir, "designs")


def _sanitize_design_name(name):
    """Convert config name to design dir name (lowercase, underscores)."""
    import re
    s = name.lower().strip()
    s = re.sub(r'[\s\-]+', '_', s)
...
def generate_config_csv(config_name):
    ...
    # Output to design dir: designs/{sanitized_name}/Simulation.data.csv
    design_dir_name = _sanitize_design_name(exact_config_name)
    design_path = os.path.join(DESIGNS_DIR, design_dir_name)
    csv_path = os.path.join(design_path, "Simulation.data.csv")
