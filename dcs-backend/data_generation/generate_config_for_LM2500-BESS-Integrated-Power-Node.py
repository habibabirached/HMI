...
def main():
    ...
    saved_configs_dir = os.path.join(backend_dir, 'saved_configs')
    os.makedirs(saved_configs_dir, exist_ok=True)
    
    # Use safe filename (replace spaces and special chars)
    safe_name = config['name'].replace(' ', '_').replace('-', '_').lower()
    output_path = os.path.join(saved_configs_dir, f'{safe_name}.json')
