...
Configurations are stored in the database and also written to the per-design structure (`designs/{design_dir}/{design_dir}.conf.json`). This makes it easy to inspect or edit configurations and supports the Save As workflow (copying a design directory).
