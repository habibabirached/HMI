...
const filterColumns = (dataArray) => {
  ...
  for(let i=0; i< dataArray.length; i=i+resolution ) {
    ...
    columns.push({
      ...
      "Ia_gen (kA)":data["Ia_gen (kA)"],
      "Ib_gen (kA)": data["Ib_gen (kA)"],
      "Ic_gen (kA)": data["Ic_gen (kA)"],
    })
  }
  return columns;
}
...
const GeneratorInstCurrentChartComponent = ({ dataArray, run, animationDuration }) => {

  return (
    <div style={{ textAlign: 'center' }}>
      <Typography sx={{ fontSize: '1.2rem', fontWeight: 'bold', marginBottom: '2px' }}> Inst. Current Generator (kA) </Typography>
      <ResponsiveContainer minWidth={CHART_MIN_WIDTH} minHeight={CHART_MIN_HEIGHT}  
        ...
        >
          ...
          {run && (<div>
              <Line name="Ia_gen (kA)" type="monotone"
                    dataKey={(dataObj) => dataObj["Ia_gen (kA)"]} 
                    stroke="#3b82f6" strokeWidth={2} dot={false} animationDuration={animationDuration} />
              <Line name="Ib_gen (kA)" type="monotone"
                    dataKey={(dataObj) => dataObj["Ib_gen (kA)"]}  
                    stroke="#8b5cf6" strokeWidth={2} dot={false} animationDuration={animationDuration} />
              <Line name="Ic_gen (kA)" type="monotone"  
                    dataKey={(dataObj) => dataObj["Ic_gen (kA)"]}  
...
export default GeneratorInstCurrentChartComponent;
